...
$sql = "SELECT id,first_name,surname,account_number,account_type,occupation,residential_address,customer_picture,phone_number FROM nobs_registration WHERE account_number LIKE  '$searchtext'";
//accountid,accountnumber,customerpicture,occupation,residentialaddress,accounttypes
if ($result = $mysqli -> query($sql)) {
  while ($row = $result -> fetch_row()) {
    echo $row[0].'___'. $row[1].'___'.$row[2].'___'.$row[3].'___'.$row[4].'___'.$row[5].'___'.$row[6].'___'.$row[7].'___'.$row[8].'*****';
