...
function loan_repaymentjvs(to,msg,customername =''){
    ...
    let themessageprint = '<br/><br/>Loan Acc No:' + account_number + '<br/><br/>' + 'Customer Name: ' + customername;
    ...
    if(checkdepositfields()){
        $.ajax({
            url: "<?php echo e(env('NOBS_IMAGES')); ?>loan_repayment.php",
            method: "POST",      // The HTTP method to use for the request
            dataType: "html",   // The type of data that you're exerciseecting back 	
            data: {  
                __id__:__id__,
    ...
    if(response == 'ERROR'){
        ...
        sndmsg(to,msg + themessageprint,true);
        //printdeposit(microfinancename,transname,acc_no,customer_name,amount,balance,currency)
//printwithdrawal
 
         //printloan_repayment('GCI SUSU','LOAN REPAYMENT',account_number,'customername',amount,'4000','GHS');
...
function makedepositjvs(to,msg,customername=''){
...
function makewithdrawaljvs(to,msg,customername=''){
...
showhidediv('loadingdiv');
 ...
 $('#modalbody2').html('<div style="padding:10px 10px;"><label>Enter 4 Digit Code Sent to: <strong>' + to +'</strong></label><input type="text" id="confirmationcode"  class="form-control"  style="width:100%;height:50px;border-radius:3px;padding:2px;font-size:16px;margin-bottom:5px;" /><input type="button" id="confirmationcodebtn" onclick="checksentcodetrue()" class="btn btn-dark" value="Pay Now" /></div>');
...
<?php /**PATH /home/bultepml/app.bultzee.com/sabs/resources/views/layouts/datascripts.blade.php ENDPATH**/ ?>